...
export function App() {
  ...
  return (
    <AdminLayout route={route}>
      <RouteContent route={route} token={auth.token} />
    </AdminLayout>
  );
}

function RouteContent({ route, token }: { route: AdminRoute; token: string }) {
  if (route.name === "orders") {
    return <OrdersPage token={token} />;
  }

  if (route.name === "order-detail") {
    return <OrderDetailPage orderId={route.orderId} token={token} />;
  }
  ...
  if (route.name === "product-edit") {
    return <ProductFormPage productId={route.productId} token={token} />;
  }
...
function DashboardPage({ token }: { token: string }) {
  ...
  async function loadDashboard() {
    try {
      setIsLoading(true);
      setError(null);
      const [orderList, productList] = await Promise.all([fetchAdminOrders(token), fetchAdminProducts(token)]);
      setOrders(orderList);
...
function OrderDetailPage({ orderId, token }: { orderId: string; token: string }) {
  ...
  async function loadOrder() {
    try {
      setIsLoading(true);
      setError(null);
      setOrder(await fetchAdminOrder(orderId, token));
    } catch {
  ...
  async function handleStatusUpdate(status: StaffOrderStatus) {
    ...
    try {
      ...
      setOrder(await updateAdminOrderStatus(order.order_id, status, token));
    } catch {
...
function ProductsPage({ token }: { token: string }) {
  ...
  async function handleToggle(product: Product) {
    try {
      ...
      const updatedProduct = await updateAdminProductActive(product.id, !product.active, token);
...
function ProductFormPage({ productId, token }: { productId?: number; token: string }) {
